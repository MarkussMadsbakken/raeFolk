...
export default function Quote(props: Readonly<QuoteProps>) {
    ...
    useEffect(() => {
        ...
        //adds an event listener to the window to check if a click is outside the reaction picker
        if (newReactionPickerOpen) {
            window.addEventListener("click", checkClick);
        }

        //checks if a click is outside the reaction picker
    ...
    const handleQuoteClick = (e: Me<HTMLDivElement, MouseEvent>) => {
        if (!props.onClick) {
            return
        }
    ...
    return (
        <div className='p-2 m-2 w-full h-full'>
            <div className={`relative grid w-full max-w-full transition-all duration-500 h-full`} onClick={(e) => handleQuoteClick(e)}>
                <div className={`absolute rounded-lg ease-in-out inset-0 bg-gradient-to-br from-${color} via-${color2} to-${color3} transition-transform bg-size-200 ${theme === "light" ? "opacity-0 bg-pos-0" : "opacity-100 bg-pos-100"} duration-1000`}></div>
                ...
                <div className='relative transition-all duration-1000 self-center'>
                ...
                <div className='flex z-10 align-middle justify-center flex-row text-xs md:right-1 md:bottom-1 md:absolute md:text-base' ref={reactionsRef}>
